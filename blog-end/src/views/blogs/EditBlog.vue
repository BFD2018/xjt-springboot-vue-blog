    ...
    <div class="edit-blog-view">
        <el-form ref="editForm"
            ...
            <el-form-item label="文章标签">
                <el-select v-model="editForm.tag_name" disabled multiple placeholder="请选择Tag标签">
                    <el-option
            ...
            <el-form-item label="分类专栏">
                <el-select v-model="editForm._type" disabled placeholder="请选择文章分类Category">
                    <el-option
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            initBlog() {
                /*通过blog_id获取博客内容*/
                this.$getRequest("/blog/getById/?bid=" + this.bid).then(res => {
                    console.log(res);
                    if (res.data.status === 200) {
                        this.editForm = res.data.obj;
                        this.editForm.flag = res.data.obj.flag === "转载" ? 0 : 1;
                        this.editForm.published = res.data.obj.published ? 1 : 0;
                    }
                })
                ...
                /*this.$request("/type/all").then(res =>{
                  ...
                  if(res.status === 200){
                    ...
                    res.obj.forEach((item,index) =>{
                      if(item.id === this.editForm.type_id){
                        console.log("item===>");
                        console.log(item);
                        this.editForm._type = item.name
                      }
                    })
                    this.typeList.push(...res.obj)
                  }
                })*/
            },
            //提交更新
            submitBlogForm() {
                ...
                this.$refs.editForm.validate((valid) => {
                    if (valid) {
                        this.$postRequest("/blog/update", this.editForm).then(res => {
                            console.log(res);
                            if (res.data.status === 200) {
                                this.$notify.success(res.data.msg);
                                this.$router.push("/admin/allblog");
                                //window.location.href = "/admin/allblog";
                            }
                        })
                    } else {
